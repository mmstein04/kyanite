...
# =============================================================================
# PARAMETERS — edit this section for each new grain / radius
# =============================================================================
...
CL_FILENAME       = f'{GRAIN_ID}_CL_registered.tif'         # 16-bit grayscale, used for the regression math
CL_COLOR_FILENAME = f'{GRAIN_ID}_CL_registered_color.tif'   # native color, display only
...
# False or if CL_COLOR_FILENAME isn't found. Never affects the regression
# math, which always uses the grayscale CL loaded from CL_FILENAME.
USE_COLOR_DISPLAY = True

# Folder of EPMA/XRF element map TIFFs (same folder CL_EPMA_registration.m
# used for this grain). All *.tif files auto-discovered.
EPMA_DIR = _REPO_ROOT / 'inputs' / 'maps' / GRAIN_ID

# --- Spatial calibration -----------------------------------------------------
EPMA_PIXEL_UM = 2.0    # µm/pixel — must match the value used during registration
...
# pixel sizes; converted to px below using EPMA_PIXEL_UM).
...
# regression is computed there; windows with fewer valid pixels are set to
# NaN in the output maps. Default: half the full disk area.
...
def main():
    ...
    if (min_rows, min_cols) != (nrows, ncols):
        raise ValueError(f'EPMA map grid ({min_rows} x {min_cols}) does not match registered '
                          f'CL size ({nrows} x {ncols}).\nEPMA_DIR may not be the folder used '
                          f'during registration for this grain.')
