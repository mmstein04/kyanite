...
def saturation_mask(x, label, verbose=True):
    # verbose=False lets a second caller (the spatial QC figure, which
    # re-derives the same mask from the raw TIFF) skip reprinting a warning
    # filtered_xy already issued for this element. See kyanite_outliers.py
    # for the method itself, shared with kyanite_pca.py.
    if not SATURATION_FILTER:
        return np.zeros(len(x), dtype=bool)
    return _saturation_mask(x, label, SATURATION_BAND_FRAC, SATURATION_MIN_FRAC,
                             SATURATION_MIN_COUNT, verbose=verbose)
